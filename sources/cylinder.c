...
/*   Updated: 2020/05/22 16:06:09 by lafontai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

void		find_cylinder_base(t_intersec *inter, t_shape *cy, t_plane plane)
{
	float d_dot_n;
	float t;
		...
		*inter->ray->direction, t))) < (cy->diameter / 2))
	{
		inter->shape = cy;
		inter->t = t;
		copy_vector(inter->normal, plane.normal);
	}
}

float		check_cylinder_base(t_plane plane, t_vector *inter, t_vector n)
{
	float d_dot_n;
	float t;
...
t_vector	cylinder_normal(t_vector *inter, t_shape *cy, float t3)
{
	...
	n = vector_diff(*inter, get_point(*cy->center, *cy->normal,
...
int			cylinder_intersection(t_intersec *intersec, t_shape *cy)
{
	...
	if (c.t3 >= 0 && c.t3 <= cy->height && c.t4 >= 0 && c.t4 <= cy->height)
	{
		intersec->shape = cy;
		intersec->t = c.t;
		copy_vector(intersec->normal, cylinder_normal(&c.inter, cy, c.t3));
	}
	return (0);
}
