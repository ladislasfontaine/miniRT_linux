...
/*   Updated: 2020/05/20 12:10:46 by lafontai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bitmap_file_header(t_scene *scene, int padding_size, int fd)
{
	...
	if (!(file_header = ft_calloc(BMP_FILE_HDR_SIZE, sizeof(unsigned char))))
		error_and_quit(scene, "Malloc failed");
	file_size = BMP_FILE_HDR_SIZE + BMP_INFO_HDR_SIZE
	...
	file_header[10] = (unsigned char)(BMP_FILE_HDR_SIZE + BMP_INFO_HDR_SIZE);
	write(fd, file_header, BMP_FILE_HDR_SIZE);
...
void	bitmap_info_header(t_scene *scene, int fd)
{
	...
	if (!(info_header = ft_calloc(BMP_INFO_HDR_SIZE, sizeof(unsigned char))))
		error_and_quit(scene, "Malloc failed");
	...
	write(fd, info_header, BMP_INFO_HDR_SIZE);
...
void	create_bmp_image(t_scene *scene, char *file_name)
{
	...
	while (i > 0)
	{
		write(fd, img->data + ((i - 1) * scene->res->w * 32 / 8)
				, img->size_line);
